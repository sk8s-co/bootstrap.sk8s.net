...
PROVIDER_TUNNEL="${PROVIDER_TUNNEL:-${1:-cloudflare}}"
shift || true

if [ "$PROVIDER_TUNNEL" = "cloudflare" ]; then
    if [ "$#" -lt 1 ]; then
        echo "Usage: $0 cloudflare <port> [cloudflared-args...]" >&2
        sleep infinity
    fi
    ...
    echo "Unsupported tunnel provider: ${PROVIDER_TUNNEL}" >&2
    sleep infinity
fi
