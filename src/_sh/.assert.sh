#!/bin/sh
set -eu

{
    # Pretty print the execution
    echo ".assert.sh (bootstrap.sk8s.net) >>>"
    # Critical requirement: /etc/resolv.conf must exist for DNS configuration
    if [ ! -f /etc/resolv.conf ]; then
        echo "Error: /etc/resolv.conf is not available."
        echo ""
        echo "This file is required to configure cluster DNS settings."
        echo "Docker should mount this automatically. Check your container runtime."
        echo ""
        exit 1
    else
        echo "  ✅ /etc/resolv.conf is present"
    fi
    
    _FAILURES=""
    
    # Check for required volume mount
    if [ ! -d "/var/run/kube" ]; then
        _FAILURES="${_FAILURES} -v /var/run/kube:/var/run/kube"
    else
        echo "  ✅ /var/run/kube volume mount"
    fi
    
    # Check for privileged mode by testing access to /dev/kmsg
    # This device is only accessible in privileged containers
    if [ ! -r /dev/kmsg ]; then
        _FAILURES="${_FAILURES} --privileged"
    else
        echo "  ✅ Privileged mode"
    fi
    
    # Check for network host mode
    if [ "$(readlink /proc/1/ns/net 2>/dev/null)" != "$(readlink /proc/self/ns/net 2>/dev/null)" ]; then
        _FAILURES="${_FAILURES} --network=host"
    else
        echo "  ✅ Network host mode"
    fi
    
    # Check for PID host mode
    # With --pid=host, /proc/1 is the host's init process, not our container entrypoint
    _PID1_COMM=$(cat /proc/1/comm 2>/dev/null || echo "")
    if [ "$_PID1_COMM" = "_start" ]; then
        _FAILURES="${_FAILURES} --pid=host"
    else
        echo "  ✅ PID host mode"
    fi
    
    # Check for IPC host mode
    # Compare our IPC namespace with PID 1's (the host init)
    if [ "$(readlink /proc/1/ns/ipc 2>/dev/null)" != "$(readlink /proc/self/ns/ipc 2>/dev/null)" ]; then
        _FAILURES="${_FAILURES} --ipc=host"
    else
        echo "  ✅ IPC host mode"
    fi
    
    # Report all failures at once
    if [ -n "$_FAILURES" ]; then
        echo "Error: Container is not configured correctly."
        echo ""
        echo "Missing required settings:"
        echo " ${_FAILURES}"
        echo ""
        exit 1
    fi
} >&2
